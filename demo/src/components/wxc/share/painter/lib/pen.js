...
export default class Painter {
  ...
  _preProcess(view, notClip) {
    ...
    let x = view.css && view.css.right ? this.style.width - view.css.right.toPx(true) : (view.css && view.css.left ? view.css.left.toPx(true) : 0);
    let y = view.css && view.css.bottom ? this.style.height - height - view.css.bottom.toPx(true) : (view.css && view.css.top ? view.css.top.toPx(true) : 0);

    if (view.key && view.type === 'text') {
      this.onpreProcess({ key: view.key, position: { x: x, y: y }, extra: extra}, function({ left, top }) {
        x = left.toPx(true)|| 0;
        y = top.toPx(true)|| 0;
      })
    }
    // if (view.type == 'text' && typeof view.autoAdapt == 'function') {
    //   const { x = left.toPx(true)|| 0, y = top.toPx(true)|| 0 } =  view.autoAdapt({ position: { x: x, y: y }, extra: extra});
    // }

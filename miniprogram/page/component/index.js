Page({
  ...
  showRandomImage: function() {
    const that = this;
    wx.request({
        url: 'http://127.0.0.1:5000/api/random-image', // 你的API地址
        success: function(res) {
            if (res.statusCode === 200) {
                that.setData({
                    imageSrc: 'http://127.0.0.1:5000/api/random-image/' + res.data // 假设后端返回的是图片URL
                });
            } else {
                console.error('Failed to fetch random image:', res);
            }
        },
        fail: function(error) {
